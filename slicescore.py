import nrrd
import sys, warnings
from numpy import int,round,linspace, newaxis, shape, array, uint32, uint8, max, sqrt, abs, mean, dtype, int32, add, divide, subtract, sum, square, multiply, asarray, squeeze, float128, average
...
def meanOverlapCoeff(data1,data2):
    """Returns the min Overlap Coefficent between image slices."""
    R=[]
    print shape(data1)
    for i in range(0,min(shape(data1))):
